    ...
    setHandler() {
        ...
        sock?.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'close') {
                ...
                const arrWh = ['all', 'connection', 'connection.update', 'connection:open']
                if (
                    arrWh.some((e) => config.webhookAllowedEvents.includes(e))
                )
                    await this.SendWebhook(
                        'connection',
                        {
        ...
        sock?.ev.on('presence.update', async (json) => {
            const arr = ['all', 'presence', 'presence.update']
            if (
                arr.some((e) => config.webhookAllowedEvents.includes(e))
            )
